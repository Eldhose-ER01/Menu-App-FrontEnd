...
export default function Menu() {
  ...
  //data take from server
  const finddatas = async () => {
    try {
      const response = await axios.get(
        "https://menu-app-backend-3.onrender.com/alldatas"
      );
  ...
  return (
    <div>
      ...
      <div className="main-btn">
        {uniqueMenuNames.map((name, index) => (
          <button key={index} onClick={() => filterData(name)}>
            {name}
          </button>
        ))}
      </div>
      ...
      <div className="menu-itmes">
        <div className="menu-box">
          ...
          <div className="menu-title">{<h1>Let's Enjoy</h1>}</div>
          <div></div>
